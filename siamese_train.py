...
class Config():
    training_dir = "/data_disk/ds_ro/discarded_object_dataset/imgs/AVSS_AB_EVAL_divx"
    testing_dir = "/data_dGisk/ds_ro/discarded_object_dataset/imgs/AVSS_AB_Hard_Divx"
...
siamese_dataset = SiameseTriplet(imageFolderDataset=folder_dataset,
                                 transform=transforms, should_invert=False)  # Get dataparser class object
...
train_dataloader = DataLoader(siamese_dataset, shuffle=True, num_workers=14,
                              batch_size=Config.train_batch_size)  # PyTorch data parser obeject
...
for epoch in range(0, Config.train_number_epochs):
    for i, data in enumerate(train_dataloader, 0):
        ...
        anchor_out, positive_out, negative_out = net(
            anchor, positive, negative)  # Model forward propagation
