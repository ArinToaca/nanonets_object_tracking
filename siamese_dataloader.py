...
class SiameseTriplet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        all_files_in_class = glob.glob(
            self.imageFolderDataset.root+anchor_class_name+'/*')
        ...
        if anchor_class_name != positive_image.split('/')[-2]:
            # Checking if the class of both anchor and positive image is same
            print("Error")
...
if __name__ == '__main__':
    ...
    siamese_dataset = SiameseTriplet(
        imageFolderDataset=folder_dataset, transform=transforms, should_invert=False)
    ...
    concatenated = torch.cat(
        (example_batch[0], example_batch[1], example_batch[2]), 0)
