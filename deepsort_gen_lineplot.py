...
VIDEO_PATH, CSV_PATH, CSV_OUT_PATH = sys.argv[1], sys.argv[2], sys.argv[3]
...
events_list = [] # how many abandonments per frame
timestamp_list = []
print(
...
for count in tqdm.tqdm(range(vid_length)):
    ...
    if count % fps == 0:
        seconds_count += 1
        events_list.append(event_count)
        timestamp_list.append(str(datetime.timedelta(seconds=seconds_count)))
        event_count = 0
    ...
    x = len([track for track in online_luggage_tracks if track.track_id in truly_abandoned_by or track.track_id in suspicious_recovered_by])
    event_count += x
...
print(f'Writing csv at {CSV_OUT_PATH}')
d = {'timestamp': timestamp_list, 'events': events_list}
